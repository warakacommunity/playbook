...
const PENDING_CHAPTERS_KEY = 'masakhane_pb_pending_chapters';
...
// Within content-only mode, allow the sync-safe subset of structural edits:
// rename a page (writes sidebar_label) and add/upload a page into an
// autogenerated chapter (a group with `addDir`). Both reflect in the real
// sidebar after the PR merges + prebuild regenerates structure.json. Moves,
// deletes, and section changes stay off — they can't reflect without rewriting
// the hand-maintained sidebars.js.
const CONTENT_STRUCTURE_EDITS = true;

// A section can accept new pages when it's an autogenerated folder (addDir,
// file-only) or an explicit leaf chapter whose pages share one folder
// (addFolder, needs a sidebars.js append).
const isAddable = (node) => !!(node.addDir || node.addFolder);

// Real docs folder a section writes into: autogenerated chapters expose their
// folder via `addDir`; explicit leaf chapters via `addFolder`; structural-mode
// nodes use their path directly.
const sectionFolder = (node) =>
  node.addDir ? `docs/${node.addDir}` : node.addFolder ? `docs/${node.addFolder}` : node.path;

// Label for a not-yet-committed page: prefer its sidebar_label / title / H1,
// else a title-cased filename.
function pendingLabel(path, content = '') {
  const fm = content.match(/^---\n([\s\S]*?)\n---/);
  const field = (k) => (fm && (fm[1].match(new RegExp(`^${k}:\\s*(.+)$`, 'm')) || [])[1]);
  const raw = field('sidebar_label') || field('title') || (content.match(/^#\s+(.+)$/m) || [])[1];
  if (raw) return raw.trim().replace(/^["']|["']$/g, '');
  return path.split('/').pop().replace(/\.mdx?$/, '').replace(/[-_]/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());
}

// Show not-yet-merged new chapters as (empty) section nodes under their parent,
// so the contributor sees the chapter appear. Their pages are filled in by
// injectPendingAdds afterwards (the new chapter carries addDir, so its folder
// matches). A chapter is dropped once its first page is no longer pending.
function injectPendingChapters(nodes, pendingChapters, changes) {
  const active = (pendingChapters || []).filter((c) => changes[c.pagePath]?.op === 'upsert');
  if (!active.length) return nodes;
  const byParent = {};
  for (const c of active) {
    (byParent[c.parentPath] ||= []).push({
      type: 'section', slug: c.label, path: `group:pending:${c.folder}`, label: c.label,
      ready: true, pending: true, addDir: c.folder, children: [],
    });
  }
  return (function rebuild(ns) {
    return ns.map((n) => {
      if (n.type !== 'section') return n;
      const kids = rebuild(n.children);
      const add = byParent[n.path];
      return { ...n, children: add ? [...kids, ...add] : kids };
    });
  })(nodes);
}

// The content-only tree comes from structure.json and so is blind to uncommitted
// work. Inject each newly-added page (an upsert whose path isn't already in the
// tree) under the section whose folder it was placed in, so the contributor sees
// it appear where they added it — matched to the first section with that folder.
function injectPendingAdds(nodes, changes) {
  const existing = new Set();
  (function walk(ns) { for (const n of ns) { if (n.type === 'page') existing.add(n.path); else if (n.children) walk(n.children); } })(nodes);
  const sections = [];
  (function walk(ns) { for (const n of ns) { if (n.type === 'section') { sections.push(n); walk(n.children); } } })(nodes);

  const bySection = {};
  for (const [path, c] of Object.entries(changes || {})) {
    if (c.op !== 'upsert' || !path.startsWith('docs/') || !/\.mdx?$/.test(path) || existing.has(path)) continue;
    const dir = path.slice(0, path.lastIndexOf('/'));
    const sec = sections.find((s) => sectionFolder(s) === dir);
    if (!sec) continue;
    (bySection[sec.path] ||= []).push({
      type: 'page', path, slug: path.split('/').pop().replace(/\.mdx?$/, ''),
      label: pendingLabel(path, c.content), ready: true, children: [], pending: true,
    });
  }
  if (!Object.keys(bySection).length) return nodes;

  return (function rebuild(ns) {
    return ns.map((n) => {
      if (n.type !== 'section') return n;
      const kids = rebuild(n.children);
      const add = bySection[n.path];
      return { ...n, children: add ? [...kids, ...add] : kids };
    });
  })(nodes);
}
...
function structureToTree(nodes) {
  return (nodes || []).map((n) =>
    n.type === 'doc'
      ...
      : { type: 'section', slug: n.label, path: `group:${n.label}`, label: n.label, ready: true, addDir: n.addDir, addFolder: n.addFolder, sidebarAnchor: n.sidebarAnchor, chapterAnchor: n.chapterAnchor, children: structureToTree(n.items) },
  );
}
...
export function StructureEditorContent({ onClose }) {
  ...
  // New chapters added under a section this session (folder + first page). Kept
  // separately so they can be rendered in the tree before the PR merges; a
  // chapter is dropped once its first page is no longer a pending change.
  const [pendingChapters, setPendingChapters] = useLocalStorage(PENDING_CHAPTERS_KEY, []);
  ...
  const tree = useMemo(
    () => (CONTENT_ONLY
      ? injectPendingAdds(injectPendingChapters(structureToTree(structure?.tree), pendingChapters, changes), changes)
      : computeDocsTree(gitFiles, catData, changes)),
    [structure, gitFiles, catData, changes, pendingChapters],
  );
  ...
  async function handleUploadFile(e) {
    ...
    // Content-only mode can only add pages into autogenerated chapters (addDir),
    // so default the target there and never offer the section/subsection
    // placements (which would need a sidebars.js edit to appear).
    const defaultSection = CONTENT_ONLY
      ? (flatSections(tree).find(isAddable)?.path ?? '')
      : (tree.find(n => n.type === 'section')?.path ?? '');
    ...
    if (allItems.length === 1) {
      ...
      setUploadPlacement(CONTENT_ONLY ? 'page' : 'section');
      setUploadTargetSection(defaultSection);
      setUploadSectionName('');
    }
  }
  ...
  // Addable chapters (autogenerated folders) labelled with their sidebar path,
  // e.g. "Foundations › Data Collection", so the upload target matches what the
  // contributor sees in the sidebar rather than a bare flat list.
  function addableSectionOptions(nodes, trail = []) {
    const out = [];
    for (const n of nodes) {
      if (n.type !== 'section') continue;
      const here = [...trail, n.label];
      if (isAddable(n)) out.push({ path: n.path, label: here.join(' › ') });
      out.push(...addableSectionOptions(n.children, here));
    }
    return out;
  }

  // Explicit leaf chapters hardcode their pages in sidebars.js, so a new file is
  // invisible until its id is listed there. Insert the id on its own line right
  // after `sidebarAnchor` (an existing, unique direct member) — append-only, so
  // nothing is reordered or removed. The patched sidebars.js rides along in the
  // same PR; getPageContent accumulates edits so multiple adds compound safely.
  async function appendSidebarEntry(section, newDocId) {
    if (!section.sidebarAnchor) return;
    const src = await getPageContent('sidebars.js');
    if (src.includes(`'${newDocId}'`) || src.includes(`"${newDocId}"`)) return; // already listed
    const esc = section.sidebarAnchor.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const anchorLineRe = new RegExp(`^([ \\t]*)(['"])${esc}\\2\\s*,?[ \\t]*$`, 'm');
    const m = src.match(anchorLineRe);
    if (!m) throw new Error(`Could not update sidebars.js: anchor "${section.sidebarAnchor}" not found`);
    const indent = m[1];
    const anchorLine = m[0].replace(/[\s,]*$/, ',');   // guarantee a trailing comma
    const newLine = `${indent}'${newDocId}',`;
    upsert('sidebars.js', src.replace(m[0], `${anchorLine}\n${newLine}`));
  }
  ...
  async function handleConfirmUpload() {
    ...
    if (uploadPlacement === 'page') {
      ...
      const path = `${sectionFolder(sectionNode)}/${pageSlug}.md`;
      const position = sectionNode.children.length + 1;
      upsert(path, buildUploadContent(markdown, title, position));
      if (sectionNode.addFolder) {
        try { await appendSidebarEntry(sectionNode, `${sectionNode.addFolder}/${pageSlug}`); }
  ...
  async function handleConfirmBatch() {
    ...
    if (uploadPlacement === 'page') {
      ...
      for (let i = 0; i < uploadBatch.length; i++) {
        ...
        const path = `${sectionFolder(sectionNode)}/${slug}.md`;
        upsert(path, buildUploadContent(item.markdown, item.title, sectionNode.children.length + 1 + i));
        if (sectionNode.addFolder) {
          try { await appendSidebarEntry(sectionNode, `${sectionNode.addFolder}/${slug}`); }
          catch (e) { setError(e.message); return; }
        }
      }
  ...
  async function addPage(sectionNode, label) {
    const slug = slugify(label);
    const position = sectionNode.children.length + 1;
    const path = `${sectionFolder(sectionNode)}/${slug}.md`;
    upsert(path, starterPage(label, position));
    // Explicit leaf chapters also need the page listed in sidebars.js.
    if (sectionNode.addFolder) {
      try { await appendSidebarEntry(sectionNode, `${sectionNode.addFolder}/${slug}`); }
      catch (e) { setError(e.message); return; }
    }
  ...
  // Insert a brand-new autogenerated chapter into a container section's items
  // array in sidebars.js, anchored on the section's unique overview id. Places
  // the new chapter first in the section; append-style, nothing else is touched.
  async function insertChapterInSidebar(section, label, folder) {
    const src = await getPageContent('sidebars.js');
    const esc = section.chapterAnchor.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const idIdx = src.search(new RegExp(`(['"])${esc}\\1`));
    if (idIdx < 0) throw new Error(`Could not update sidebars.js: section anchor "${section.chapterAnchor}" not found`);
    const arrIdx = src.indexOf('[', idIdx);
    if (arrIdx < 0) throw new Error('Could not locate the section items array in sidebars.js');
    const indent = (src.slice(arrIdx + 1).match(/\n([ \t]+)\S/) || [, '      '])[1];
    const insertion = `\n${indent}chapter(${JSON.stringify(label)}, ${JSON.stringify(folder)}),`;
    upsert('sidebars.js', src.slice(0, arrIdx + 1) + insertion + src.slice(arrIdx + 1));
  }

  async function addChapter(sectionNode, label) {
    const folder = slugify(label);
    const pagePath = `docs/${folder}/index.md`;
    try {
      await insertChapterInSidebar(sectionNode, label, folder);
    } catch (e) {
      setError(e.message);
      return;
    }
    upsert(pagePath, starterPage(label, 1));
    setPendingChapters(prev => [
      ...prev.filter(c => c.folder !== folder),
      { parentPath: sectionNode.path, folder, label, pagePath, pageLabel: label },
    ]);
    setExpanded(prev => new Set([...prev, sectionNode.path, `group:pending:${folder}`]));
    const { frontmatter, content } = splitFrontmatter(starterPage(label, 1));
    setRightPanel({ path: pagePath, htmlContent: mdToHtml(content), frontmatter, fetching: false, dirty: false });
  }
            ...
            >
              ...
              <div className={`${styles.leftPanel}${leftHidden ? ` ${styles.leftPanelHidden}` : ''}`}>
                ...
                <div className={styles.treePanelHeader}>
                  ...
                  {(!CONTENT_ONLY || CONTENT_STRUCTURE_EDITS) && (
                  <div className={styles.treePanelHeaderActions}>
                    ...
                    <button
                      ...
                      disabled={!auth}
                      title={!auth ? 'Sign in with GitHub to upload a document' : 'Upload a document as a new page in a chapter'}
                      type="button"
                  ...
                  <div className={styles.treeScroll}>
                    {tree.map(node => (
                      <TreeRow
                        ...
                        structureEdits={CONTENT_STRUCTURE_EDITS}
                      />
                    ))}
              ...
              <div className={styles.uploadPreviewFooter}>
                <div className={styles.uploadPlacement}>
                  ...
                  {!CONTENT_ONLY ? (
                    ...
                    <span style={{ fontSize: '0.85rem', opacity: 0.8 }}>New page in an editable chapter</span>
                  )}
                  ...
                  {(uploadPlacement === 'page' || uploadPlacement === 'subsection') && (
                    <div className={styles.uploadSectionPicker}>
                      <label className={styles.uploadTitleLabel}>
                        {CONTENT_ONLY ? 'Chapter' : uploadPlacement === 'page' ? 'Section' : 'Parent section'}
                      </label>
                      <select
                      ...
                      >
                        {(CONTENT_ONLY ? addableSectionOptions(tree) : flatSections(tree)).map(n => (
                  ...
                  {!CONTENT_ONLY && (uploadPlacement === 'section' || uploadPlacement === 'subsection') && (
                    <div className={styles.uploadSectionPicker}>
              ...
              <div className={styles.uploadPreviewFooter}>
                <div className={styles.uploadPlacement}>
                  ...
                  {!CONTENT_ONLY ? (
                    ...
                    <span style={{ fontSize: '0.85rem', opacity: 0.8 }}>New pages in an editable chapter</span>
                  )}
                  {(uploadPlacement === 'page' || uploadPlacement === 'subsection') && (
                    <div className={styles.uploadSectionPicker}>
                      <label className={styles.uploadTitleLabel}>
