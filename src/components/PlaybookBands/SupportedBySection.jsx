...
export default function SupportedBySection() {
  ...
  // Resolve logo paths through baseUrl so they work under /playbook/.
  const resolveLogo = (src) => (/^https?:\/\//.test(src) ? src : withBaseUrl(src));
  return (
        ...
        <div className={styles.supportersTrack}>
          {track.map((s, idx) => (
            <a
              ...
              <img src={resolveLogo(s.logo)} alt={s.name} loading="lazy" decoding="async" />
            </a>
          ))}
        </div>
      </div>
