...
page.on('response', async r => { if (r.status() >= 400) { let t=''; try{t=(await r.text()).slice(0,160);}catch{} fails.push(`${r.status()} ${r.request().method()} ${r.url().replace('http://localhost:8010','')} :: ${t.replace(/\s+/g,' ')}`);} });
...
await page.goto('http://localhost:8010/auth/signup', { waitUntil:'networkidle' }); await page.waitForTimeout(1500);
