...
AfriAnnotate is a hosted platform; end users sign up at
[label.afriannotate.org](https://label.afriannotate.org) — no local installation. For
operators self-hosting the open-source repo, see the README at
[https://github.com/AfriAnnotate/Tool](https://github.com/AfriAnnotate/Tool) for boot instructions.

## Projects

### Blank page when loading a project

After opening a project, you see a blank page. Most commonly this is
a stale-cache issue — hard-reload (Cmd-Shift-R / Ctrl-Shift-R). If
that doesn't help, sign out and back in to mint a fresh JWT.

For self-hosters: if your `PLATFORM_HOST` env var lacks a scheme
(`http://` or `https://`), AfriAnnotate can fail to load assets.
Update the env var to include the scheme and restart.
...
* If you want to upload a large volume of data (thousands of items), consider doing that at a time when people are not labeling or use a different database backend such as PostgreSQL or Redis. You can run Docker Compose from the root directory of AfriAnnotate to use PostgreSQL: `docker-compose up -d`, or see [Sync data from cloud or database storage](/annotate/data-import/cloud-storage). 

* If you are using a labeling schema that has many thousands of labels, consider using an [external taxonomy](https://labelstud.io/tags/taxonomy) instead. 
...
* For Amazon S3, see [Configuring and using cross-origin resource sharing (CORS)](https://docs.aws.amazon.com/AmazonS3/latest/userguide/cors) in the Amazon S3 User Guide.
...
If you have not set up CORS, you cannot view cloud storage data from AfriAnnotate. You might see a link to the data rather than a preview of the data, or you might see a CORS error in your web browser console:
...
:::note
Make sure to apply the correct role and permissions for your
Service Account. For GCS source/target storage, the cloud
operator's Service Account needs the
``roles/iam.serviceAccountTokenCreator`` role (so the platform
can mint short-lived signed URLs for objects).
:::
### 403 errors
...
<details>
<summary>Google Cloud Storage credentials</summary>
...
<details>
<summary>Amazon S3 credentials</summary>


For Amazon S3, see [Configuration and credential file settings](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-files) in the Amazon AWS Command Line Interface User Guide. Also check that your credentials work from the [aws client](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-files).
...
* **Import method** should be set to `Files` for simple cases if you work with images, audio, text files or any other binary content stored in the bucket.
    ...
    This instructs AfriAnnotate to create tasks automatically with URI links (like `s3://bucket/1.jpg`), and resolve them into presigned `https` URLs when opening the labeling screen. 

    If you store JSON/JSONL tasks in the AfriAnnotate format or Parquet files in your bucket - set this option to "Tasks". 
...
If there is only LIST permission, AfriAnnotate can scan the bucket for the existence of objects without actually reading them. With GET permissions, AfriAnnotate can read the data and extract your JSON files appropriately. 


### Tasks don't load the way I expect

If the tasks sync to AfriAnnotate but don't appear the way that you expect, maybe with URLs instead of images or with one task where you expect to see many, check the following:
- If you're placing JSON files in [cloud storage](/annotate/data-import/cloud-storage), ensure that if you have multiple tasks in the same file, they are all formatted the same way (for example, you cannot have 1 task with the raw contents of the `data` field and another task that contains annotations and predictions in the same file).
...
<details>
<summary>Image annotation units</summary>


<!-- TODO: include was includes/image_units.md (not ported from Hexo) -->

</details>

### Annotators cannot see predictions

If annotators can't see predictions or if you encounter unexpected behavior after you [import pre-annotations into AfriAnnotate](/annotate/data-import/tasks), review this guidance to resolve the issues.
...
If the offsets for exported HTML labels don't match your expected output, such as with HTML named entity recognition (NER) tasks, the most common reason why is due to HTML minification. When you upload HTML files to AfriAnnotate for labeling, the HTML is minified to remove whitespace. When you annotate those tasks, the offsets for the labels apply to the minified version of the HTML, rather than the original unmodified HTML files. 

To prevent the HTML files from being minified, you can use a different import method. See [Import HTML data](/annotate/data-import/tasks#import-html-data) for more.

If you want to correct existing annotations, you can minify your source HTML files in the same way that AfriAnnotate does. The minification is performed with the following script:
...
You can investigate most problems using the server console log. The machine learning backend runs as a separate server from AfriAnnotate, so make sure you check the correct server console logs while troubleshooting. To see more detailed logs, start the ML backend server with the `--debug` option. 
...
### AfriAnnotate default timeout settings for ML server requests
...
3. Predict - prediction request when AfriAnnotate gets predictions from ML backend (env variable `ML_TIMEOUT_PREDICT`)
...
You can adjust the timeout by setting an environment variables for each request or modify in AfriAnnotate variables. These are the variables section in AfriAnnotate (in seconds):
...
### I launched the ML backend, but it appears as **Disconnected** after adding it in the AfriAnnotate UI
...
If you can't resolve the traceback issues by yourself, [contact us on Slack](https://slack.label.afriannotate.org/?source=docs-ML).
...
- Check that your ML backend's predictions follow the [pre-annotation format used in task imports](/annotate/data-import/tasks).
- Confirm that your project's label configuration matches the output produced by your ML backend. For example, use the `<Choices>` tag to create a class of predictions for text. See [the labeling-config tag reference](/annotate/labeling-config/overview).
...
Because the ML backend and AfriAnnotate are different services, the assets (images, audio, etc.) that you label must be hosted and be accessible with URLs by the machine learning backend, otherwise it might fail to create predictions.

### I get a validation error when adding the ML backend

If you get a validation error when adding the ML backend URL to your AfriAnnotate project, check the following:
...
- Is your machine learning backend available from your AfriAnnotate instance? It must be available to the instance running AfriAnnotate.

If you're running AfriAnnotate in Docker, you must run the machine learning backend inside the same Docker container, or otherwise make it available to the Docker container running AfriAnnotate. You can use the `docker exec` command to run commands inside the Docker container, or use `docker exec -it <container_id> /bin/sh` to start a shell in the context of the container. See the [docker exec documentation](https://docs.docker.com/engine/reference/commandline/exec/). 
...
For example, AfriAnnotate ML Backend library is used as 
...
You must ensure that the ML backend can access your AfriAnnotate data. If it can't, you might encounter the following issues:

* `no such file or directory` errors in the server logs.
* You are unable to see predictions when loading tasks in AfriAnnotate.
* Your ML backend appears to be connected properly, but cannot seem to complete any auto annotations within tasks. 

To remedy this, ensure your ML backend has the `LABEL_STUDIO_URL` and `LABEL_STUDIO_API_KEY` environment variables set so it can call back into AfriAnnotate.
