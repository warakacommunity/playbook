...
How to label tasks on AfriAnnotate. Before this page is useful, your
project owner should have already
[set up a project](/annotate/projects/setup),
[configured the labeling interface](/annotate/labeling-config/overview),
and [imported data](/annotate/data-import/tasks). You're added to the
project as an **Annotator** (or a higher role); you see it in the
sidebar.

Quick path:

1. Open the project from the sidebar (or **Home → Projects**).
2. Click **Label All Tasks**. The labeling stream opens at the
   next unlabeled task assigned to you.
3. Use keyboard shortcuts (or your mouse) to apply labels.
4. Click **Submit**. The next task loads.
5. Follow the project instructions panel for task-specific guidance
   (left side, expandable).

You can also annotate on a **phone or tablet** — see
[Mobile install](/annotate/mobile/install). Same projects, same accounts,
same hotkeys (where the device has a keyboard). 
...
You can also delete all annotations on a task from the Data Manager — select tasks, click the bulk-actions dropdown, and choose **Delete annotations**.

### Add relations between annotations
You can create relations between two results with both directions and labels. To add labels to directions, you must set up a labeling config with the relations tag. See more about [relations with labels](https://labelstud.io/tags/relations) in the Tags documentation.
...
- To add labels to the direction arrow indicating the relation between two annotations, click the vertical ellipsis button next to the two related regions to add your predefined labels. You must have a [label configuration that includes relations](https://labelstud.io/tags/relations) to do this.
...
If your project has a machine-learning backend that returns interactive pre-annotations, you can choose whether to use those predictions while you label. 
...
<br>
![](/annotate-assets/img/outliner-details-panels-view.png)
...
<br>
![](/annotate-assets/img/collapse-details-button.png)
...
<br>
![](/annotate-assets/img/outliner-expanded-view.png)
...
<br>
![](/annotate-assets/img/details-floating-dock.png)
...
<br>
![](/annotate-assets/img/region-editor.png)
...
:::
<br>
![](/annotate-assets/img/comments-box.png)
...
![](/annotate-assets/img/text-inside-outliner.png)
...
<br>
![](/annotate-assets/img/group-by-label-sorted-by-score-expanded-dog-icon.png)
<i>Figure 14: Group by label using sorted by score</i>

<br>
![](/annotate-assets/img/group-by-tool-sorted-by-score.png)
<i>Figure 15: Group by tool using sorted by score</i>

<br>
![](/annotate-assets/img/group-by-label-sorted-by-score.png)
<i>Figure 16: Group by label using sorted by score</i>

<br>
![](/annotate-assets/img/manual-grouping.png)
...
## How AfriAnnotate saves results in annotations

Each annotation you create on a task contains **regions** and **results**.

- **Regions** are the selected area of the data — a text span, an
  image bounding box, an audio segment, a single classification
  choice, etc.
- **Results** are the labels attached to a region.

Each region has a unique ID per annotation, formed of the characters
`[A-Za-z0-9_-]`. When one region carries multiple labels (e.g. a
bounding box that gets both a "Coffee" label and a per-region price
text), each label is its own result entry — and they all share the
same region ID.

When a prediction is used to create an annotation (you click
**Accept** on a pre-annotated region), the result IDs stay the
same in the saved annotation. This lets you track which regions
your ML model produced versus which the annotator drew from
scratch by matching IDs across the prediction and annotation
arrays.

For the full JSON shape of regions and results, see
[Task format → Result format](/annotate/data-import/task-format#result-format).
