...
description: "Control tag — The `Labels` tag provides a set of labels for labeling regions in tasks for machine learning and data science projects. Use the `Labels` tag to create a set of labels that can be assigned to identified region and specify the values of labels to assign to regions."
mdx:
  format: md
---
...
| choice | single,multiple | no | `single` | Configure whether you can select one or multiple labels for a region |
| maxUsages | number | no | — | Maximum number of times a label can be used per task |
| showInline | true,false | no | `true` | Whether to show labels in the same visual line |
| opacity | float | no | `0.6` | Opacity of rectangle highlighting the label |
...
| value | string | no | — | Task data field containing a list of dynamically loaded labels (see example below) |
