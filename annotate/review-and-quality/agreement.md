...
The following metrics are available for control tags out-of-the-box in AfriAnnotate.
...
When annotators highlight a region of text and assign it a label, AfriAnnotate records the character positions where the highlight starts and ends, along with the label. For example, in the sentence:
...
When an annotator draws a rectangle on an image and assigns it a label, AfriAnnotate records the box's position (x, y coordinates of the top-left corner) and its width and height. For example, if an annotator draws a box around a dog in a photo and labels it **Dog**, AfriAnnotate stores the box's coordinates along with that label.
...
The IoU concept applies to more than just rectangular bounding boxes. AfriAnnotate adapts the same core idea for different annotation types:
...
AfriAnnotate offers two alternative metrics for `Taxonomy` tags that provide partial credit, unlike Exact Match which requires the full taxonomy selection to be identical. Both are useful when you want to recognize that two annotators who selected nearby items in a taxonomy are in closer agreement than two who selected completely unrelated items.
...
If none of the built-in metrics fits your data, write your own in
Python. AfriAnnotate exposes a metric-registry interface where
operators with `is_staff` can drop in a callable that receives two
annotation results and returns a 0..1 agreement score.

The shape:

```python
# Per-annotation-result agreement function. Receives the two ``result``
# entries to compare; returns a float between 0 and 1, or None if the
# pair is incomparable (e.g. value-shape mismatch).
def my_metric(result_a: dict, result_b: dict) -> float | None:
    # result_a and result_b each have:
    #   - from_name, to_name, type  — the tag context
    #   - value                     — tag-specific payload
    #   - id                        — region ID (same id = same region)
    a_val = result_a["value"]
    b_val = result_b["value"]
    # ... your comparison logic ...
    return some_float_between_0_and_1
```

### Add a custom metric (admin UI)

**Project → Settings → Quality → Custom metrics → + New metric**:

1. **Name** — what shows up in the metric dropdown
2. **Description** — one-line summary (for the tooltip)
3. **Code** — your Python function, pasted into a Monaco editor
4. **Test against samples** — pick 3-5 existing tasks; the UI runs
   your metric against their annotations and shows the resulting
   scores side-by-side
5. **Save**

Saved metrics live at the **project** level. To share across
projects, save it at the org level instead (**Organization →
Quality → Custom metrics**).

### Sandbox + safety

Custom metric code runs in a **restricted Python sandbox**:

- No `import` of arbitrary modules — pre-imports include `math`,
  `re`, `statistics`, and a `Levenshtein` helper
- No I/O — no file reads, no network calls, no subprocess
- Wall-clock timeout: 1 s per task pair
- Memory cap: 64 MB

If your metric needs more than this (e.g. semantic similarity via
an embedding model), use an external ML backend instead — the
metric registry can call an MLBackend endpoint and get a score back.
See the API examples below.

## Quality API

Programmatic access to metrics + agreement data:

### List available metrics

```bash
curl "https://label.afriannotate.org/api/projects/<id>/quality/metrics" \
  -H "Authorization: Bearer YOUR_PAT"
```

Returns the metric registry — built-ins + any custom metrics on
this project or its org:

```json
{
  "builtin": [
    {"name": "Exact Match",    "type": "classification"},
    {"name": "Span Overlap",   "type": "spans"},
    {"name": "IoU Rectangles", "type": "rectangles"},
    {"name": "...", "type": "..."}
  ],
  "custom": [
    {"name": "Semantic Similarity (ada-002)", "owner": "org", "id": 42}
  ]
}
```

### Recompute agreement for a project

```bash
curl -X POST "https://label.afriannotate.org/api/projects/<id>/quality/recompute" \
  -H "Authorization: Bearer YOUR_PAT" \
  -H "Content-Type: application/json" \
  -d '{"metric": "Exact Match"}'
```

Async — returns a job ID. Poll
`/api/projects/<id>/quality/recompute/<job_id>` for status.

The job recalculates `Task.precomputed_agreement` for every task in
the project. Useful when:

- You've changed which metric the project uses
- You've imported historical annotations and want them scored
- You've fixed bugs in a custom metric and need to re-score

### Get agreement summary

```bash
curl "https://label.afriannotate.org/api/projects/<id>/quality/summary" \
  -H "Authorization: Bearer YOUR_PAT"
```

Returns aggregate agreement statistics: distribution histogram,
per-annotator means, tasks below threshold, etc.

## The agreement-metrics registry (developer reference)

The full list of built-in metrics is registered in
`label_studio/projects/agreement_metrics.py`. Each metric:

- Has a `name` (what appears in the UI dropdown)
- Has a `group` (Classification, Spans, Rectangles, etc.) — used to
  organise the dropdown
- Is a callable matching the `(result_a, result_b) -> float | None`
  signature above
- Optionally declares a `tag_types` allowlist — control tags it's
  designed for. The UI still lets operators pick any metric on any
  project; the `tag_types` field only filters the dropdown for
  discoverability.

The current registry covers 20+ metrics across classification,
spans, rectangles, polygons, video, timeline, semantic similarity,
and the three taxonomy variants documented above.

To contribute a new metric upstream, add to the registry in
`agreement_metrics.py` and open a PR to [https://github.com/AfriAnnotate/Tool](https://github.com/AfriAnnotate/Tool).
