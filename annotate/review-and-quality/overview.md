...
sidebar_label: "Review annotations"
...
After multiple labelers have annotated tasks, review their output to validate the quality of the results. You can also perform this task after a model has predicted labels for tasks in your dataset. To configure the settings for reviewing annotations, see [Create a project](/annotate/projects/setup) and the **Quality** section of the project's settings.

The annotation review workflow ships with AfriAnnotate. Reviewers
are added at the project level via **Project → Members** and granted
the Reviewer (or higher) role — see [Roles](/annotate/organization/roles).
...
For AfriAnnotate projects, you can configure the review stream ordering and limit the review slice:
...
1. As an **ADMINISTRATOR**, log in to the AfriAnnotate app.
...
![](/annotate-assets/img/go-back-reviewstream.png)
...
![Screenshot of pause](/annotate-assets/img/review/pause.png)

When a user is paused, the following occurs:

* They immediately see a message informing them that they have been paused. 

    ![Screenshot of message](/annotate-assets/img/review/paused-message.png)
...
![Screenshot of hover](/annotate-assets/img/review/paused-tooltip.png)
...
You can use **Project → Settings → Quality → Annotation Limit** to cap how many tasks an annotator is able to complete before they're paused.

#### Behavior-based triggers

Behavior-based pausing (spam / bot detection plugins) is on the roadmap but not yet shipped in this release.
...
- The total agreement for one annotator with all other annotators. See [Agreement metrics](/annotate/review-and-quality/agreement) for how this number is calculated.
...
Define ground truth annotations in a AfriAnnotate project. Use ground truth annotations to assess the quality of your annotated dataset. Review ground truths to make sure that annotators are accurately labeling data at the start of the project, and continually throughout the lifecycle of the training dataset creation.

AfriAnnotate compares annotations from annotators and model predictions against the ground truth annotations for a task to calculate an accuracy score between 0 and 1.

:::note
Ground truth annotations are part of the standard quality workflow
in AfriAnnotate. Any project member with Reviewer or higher role
on the project can mark an annotation as ground truth.
:::
