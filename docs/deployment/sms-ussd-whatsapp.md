...
5. **Voice as first-class input for WhatsApp.** Text-only design misses the largest natural interaction pattern on the channel. ASR quality (per the [ASR page](../before-you-start/asr.mdx)) becomes a deployment blocker.
...
- **ASR quality** on WhatsApp voice notes is directly the [ASR chapter's](../before-you-start/asr.mdx) scope; the channel just moves ASR from "feature" to "deployment blocker."
