...
└── No published corpus for your language.
    ...
    └── Genuinely uncovered → Read the [long-tail language
        onboarding](../long-tail-language/index) chapter and
...
- **Cross-lingual transfer is unusually weak.** Cross-lingual sentiment transfer is already weak (see the [sentiment page](./sentiment)); for hate speech the gap is larger because the harm categories, in-group codewords, and cultural triggers do not transfer at all. Treat transferred hate-speech models as pilot signal, not deployment-ready.
