...
- **Retrieval models.** [mDPR (multilingual Dense Passage Retrieval)](https://arxiv.org/abs/2004.04906) and [mContriever](https://arxiv.org/abs/2112.09118) are the AfriQA-paper baselines and the honest starting point when you want to reproduce the paper's numbers. For a *stronger* retrieval baseline in 2026, **[BGE-M3](https://huggingface.co/BAAI/bge-m3)** (BAAI, 2024) is the current open-model reference for cross-lingual dense retrieval — XLM-RoBERTa-large base, 100+ languages, 8k-token context, supports dense + multi-vector + sparse retrieval simultaneously, MIT-licensed, tops MIRACL and MKQA. **[mE5-large](https://huggingface.co/intfloat/multilingual-e5-large)** and **[Cohere Embed Multilingual v3](https://cohere.com/embed)** are the other current cross-lingual retrievers worth measuring against on your target language pair. Start with the AfriQA baselines for reproducibility, then compare a modern retriever on the same test set before committing to a retrieval choice.
