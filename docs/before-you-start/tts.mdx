...
└── No — MMS-TTS does not cover your language.
    ...
    └── Genuinely uncovered → Read the [long-tail language
        onboarding](../long-tail-language/index) chapter first.
...
The [legal, consent, and community IP](../legal-consent/index) chapter is the operational reference; the section on consent from non-literate speakers applies with additional weight here.
...
- **Character-error-rate round-trip** — synthesise a test set, transcribe with an ASR (per the [ASR page](./asr)), compute CER against reference text. Useful as an intelligibility proxy.
