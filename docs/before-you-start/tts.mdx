...
*Sweep note.* The 2026-Q3 currency sweep did not surface a verified African-language displacer of the TTS recommendations on this page. The MMS / F5-TTS / StyleTTS 2 / Voicebox lineage is the workable frontier here as of mid-2026, but treat the recommendations as **unverified in this pass** rather than "confirmed unchanged" and check the current [BibleTTS](https://huggingface.co/datasets/masakhane/bibletts) and [MMS TTS model releases](https://huggingface.co/facebook/mms-tts) for newer per-language checkpoints before committing to a base.
...
- **[F5-TTS](https://arxiv.org/abs/2410.06885)** ([Chen et al., 2024](https://arxiv.org/abs/2410.06885)): flow-matching TTS with strong zero-shot voice cloning, actively maintained. Increasingly the practical default for teams that would previously have reached for XTTS, given Coqui's archival state.
- **[StyleTTS 2](https://arxiv.org/abs/2306.07691)** ([Li et al., 2023](https://arxiv.org/abs/2306.07691)): diffusion-adjacent TTS with prosody control and high naturalness. Strong on English; check per-language fine-tuning viability for your target African language before committing.
- **[Meta Voicebox](https://arxiv.org/abs/2306.15687)**: non-autoregressive TTS with in-context zero-shot generation. Research-forward release; verify licence and African-language coverage before production use.
...
- **Community fine-tunes** on the [Hugging Face Hub under `masakhane/`](https://huggingface.co/masakhane), `sunbird/`, and country-specific research groups. Coverage is growing. Model-card quality is variable; do not deploy without independently measuring MOS on your target language.
...
- **Character-error-rate round-trip**: synthesise a test set, transcribe with an ASR (per the [ASR page](./asr.mdx)), compute CER against reference text. Useful as an intelligibility proxy.
...
- [F5-TTS paper (Chen et al., 2024)](https://arxiv.org/abs/2410.06885): the flow-matching TTS architecture that has become the actively-maintained alternative to XTTS since Coqui's archival state.
- [StyleTTS 2 paper (Li et al., 2023)](https://arxiv.org/abs/2306.07691): diffusion-adjacent TTS with prosody control.
- [Meta Voicebox paper](https://arxiv.org/abs/2306.15687): non-autoregressive TTS with in-context zero-shot generation.

</details>

---

**Contributor's note.** If you are contributing to this page (new corpora, per-language MOS numbers, deployment retrospectives), preserve the voice-consent framing. It is the ethical difference between a research release and a defensible deployment.
