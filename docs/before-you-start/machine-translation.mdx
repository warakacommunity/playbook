...
**Editorial opinion.** For a new project on a language covered by NLLB-200, NLLB-200-distilled remains a defensible fine-tuning starting point in mid-2026, but treat it as one of five candidates rather than the automatic default: measure NLLB-200 vs. SeamlessM4T v2 vs. Aya-Expanse (or Tiny Aya at 3B for constrained deployment) vs. TranslateGemma vs. AfriqueLLM (AfriqueGemma / AfriqueQwen / AfriqueLlama, whichever base matches your compute) on FLORES-200 dev + a native-speaker-verified small in-domain set before committing. Fine-tune the winner. Do **not** trust FLORES numbers without human review. If your language is not in NLLB-200, the task is genuinely research-grade — read the "cross-language transfer" chapter before scoping.
